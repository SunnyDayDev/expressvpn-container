<!-- Копия docs/api.md из репозитория Detour (для установленного пакета навыка).
     При изменении API обновляйте оба файла. -->
# API агента Detour
...
Нормативный контракт — `openspec/specs/agent-api/spec.md` в репозитории Detour;
...
`docker compose exec detour detour-agent reset-password` (см. README репозитория).
