...
Агент SHALL обслуживать веб‑интерфейс и HTTP/JSON API с префиксом `/v1` на одном фиксированном порту внутри контейнера (публикация на хост — по `.env`, по умолчанию `127.0.0.1:48100`). Каждый `/v1`‑запрос, кроме `GET /healthz`, SHALL требовать либо валидную cookie‑сессию веб‑интерфейса, либо заголовок `Authorization: Bearer <token>`. API‑токен SHALL генерироваться агентом при первом старте, храниться в томе и быть доступным для просмотра/ротации в UI. Запрос без валидной аутентификации SHALL получать `401`.

#### Scenario: Missing credentials
- **WHEN** клиент вызывает `GET /v1/state` без cookie‑сессии и без заголовка Authorization
...
Агент SHALL предоставлять: `POST /v1/auth/setup` (создание пароля администратора, доступно только пока пароль не задан), `POST /v1/auth/login` (пароль → HTTP‑only cookie‑сессия), `POST /v1/auth/logout`, `POST /v1/auth/logout-all` (инвалидирует все сессии, включая текущую), `POST /v1/auth/password` (смена пароля; инвалидирует прочие сессии). Хэш пароля SHALL храниться в томе; изменяющие запросы с cookie‑сессией SHALL требовать CSRF‑токен; после 5 неверных попыток подряд SHALL действовать пауза не менее 30 секунд.

#### Scenario: Setup only once
- **WHEN** пароль уже задан и клиент вызывает `POST /v1/auth/setup`
